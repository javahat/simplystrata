...
if ( is_user_logged_in() )
	...
	echo 'Hello ' . $current_name . '. You are viewing the <strong>'. $site_name . '</strong> website with <strong>' . $current_role . '</strong> permission. '; 
	...
	// Content visible to all other logged in users (owner, strata council, admin)
		...
		{ 
		echo 'You are able to acccess private documents such as minutes and other important documents specific to owners.<br><br>'; 
		}
...
elseif (! is_user_logged_in() || ! current_user_can('read_private_pages') || $securecontent == '') 
	{
	the_content( sprintf(
